...
def poleScale(y, width, relative=True):
    ...
    scaleFactor = (1.0 / pi4) * pi2OverWidth * (
                np.cos(theta - (piOverHeight / 2.0)) - np.cos(theta + (piOverHeight / 2.0)))
    if relative:
        scaleFactor /= (1.0 / pi4) * pi2OverWidth * (
                    np.cos(piHalf - (piOverHeight / 2.0)) - np.cos(piHalf + (piOverHeight / 2.0)))
    return scaleFactor
...
def sh_visualise(lmax=2, sh_basis_matrix=None, showIt=False, outputDir='./output/'):
    ...
    fig, axs = plt.subplots(nrows=rows, ncols=cols, gridspec_kw={'wspace': 0.1, 'hspace': -0.4}, squeeze=True,
                            figsize=(16, 8))
    ...
    for l in range(0, lmax + 1):
        ...
        for i in range(0, nInBand):
            ...
            axs[l, i + colOffset].imshow(sh_basis_matrix[:, :, imgIndex],
                                         cmap=LinearSegmentedColormap('RedGreen', cdict), vmin=-1, vmax=1)
            imgIndex += 1
...
if __name__ == "__main__":
    ...
    fn = outputDir + '_diffuse_ibl_GT_' + str(resizeWidth) + '_' + str(diffuseLowResWidth) + '_' + str(
        outputWidth) + '.exr'
    diffuse_ibl_GT = getDiffuseMap(ibl_filename, width=resizeWidth, widthLowRes=diffuseLowResWidth,
